...
import {
  Plus, X, Mail, Phone, Wrench,
  CheckCircle2, XCircle, Pencil, Trash2, PowerOff, Power,
} from 'lucide-react';
...
export default function ManagerUsersPage() {
  ...
  const [confirmToggle, setConfirmToggle] = useState<{ id: string; isActive: boolean; name: string } | null>(null);
  ...
  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<Record<string, any>>();
  ...
  const closeForm = () => { setShowForm(false); setEditingUser(null); reset(); };
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <button onClick={closeForm}><X className="w-5 h-5 text-slate-400 hover:text-slate-600" /></button>
              </div>
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Nom complet *</label>
                  <input {...register('name', { required: 'Nom requis' })} placeholder="Ex: Karim Mansour"
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                  {errors.name && <p className="mt-1 text-xs text-red-600">{String(errors.name.message)}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Email *</label>
                  <input {...register('email', { required: 'Email requis' })} type="email" placeholder="tech@dataserv.tn"
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          ...
          <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => toggleMutation.mutate({ id: confirmToggle.id, isActive: confirmToggle.isActive })}
            ...
            {techniciens.map((tech: any) => (
              <div
                ...
                className={`bg-white border rounded-xl p-5 hover:shadow-sm transition-shadow ${
                  tech.isActive ? 'border-slate-200' : 'border-orange-200 bg-orange-50/30'
                }`}
              >
                {/* Header card */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${
                        tech.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-600'
                      }`}>
                        {tech.isActive
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(tech)} title="Modifier"
                      className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => setConfirmToggle({ id: tech.id, isActive: tech.isActive, name: tech.name })}
                      title={tech.isActive ? 'Désactiver' : 'Réactiver'}
                      className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                        tech.isActive
                          ? 'hover:bg-orange-50 text-slate-400 hover:text-orange-600'
                          : 'hover:bg-emerald-50 text-slate-400 hover:text-emerald-600'
                      }`}>
                      {tech.isActive ? <PowerOff className="w-3.5 h-3.5" /> : <Power className="w-3.5 h-3.5" />}
                    </button>
                    <button
                      onClick={() => setConfirmDelete({ id: tech.id, name: tech.name })}
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
                  <span>Depuis {formatDate(tech.createdAt)}</span>
