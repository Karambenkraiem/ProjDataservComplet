...
export class InterventionsService {
  ...
  private generatePdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.rect(0, 105, W, 4).fill(C.blue);             // accent stripe

      // Logo area (small square accent)
      doc.rect(L, 24, 6, 36).fill(C.blue);

      // Brand name
      doc.fillColor(C.white).fontSize(27).font('Helvetica-Bold')
         .text('DataServ', L + 14, 22);

      // Subtitle
      doc.fillColor(C.accent).fontSize(10).font('Helvetica')
         .text("Fiche d'Intervention Technique", L + 14, 57);
      ...
      doc.fillColor(C.white).fontSize(9.5).font('Helvetica-Bold')
         .text(ref, W - L - 148, 27, { width: 148, align: 'center' });

      // Date (top-right, below pill)
      doc.fillColor(C.accent).fontSize(8.5).font('Helvetica')
         .text(`Clôturé le ${closeDate}`, W - L - 148, 52, { width: 148, align: 'right' });

      // ══════════════════════════════════════════════════════════════════
      // 2. TICKET TITLE STRIP
      // ══════════════════════════════════════════════════════════════════
      doc.rect(0, 109, W, 52).fill(C.slateLight);

      // Priority badge
      doc.roundedRect(L, 122, 65, 18, 9)
         .fill(isUrgent ? C.orangeLight : C.blueLight);
      doc.fillColor(isUrgent ? C.orange : C.blue)
      ...
      // ══════════════════════════════════════════════════════════════════
      // 3. TWO-COLUMN: TICKET INFO | CLIENT INFO
      // ══════════════════════════════════════════════════════════════════
      let y = 177;
      ...
      doc.fillColor(C.white).fontSize(7.5).font('Helvetica-Bold')
         .text('INFORMATIONS DU TICKET', L + 10, y + 7);
      ...
      ticketRows.forEach(([label, value], i) => {
        ...
        doc.fillColor(C.slate).fontSize(8).font('Helvetica')
           .text(label, L + 10, ry, { width: c1W * 0.45 });
        doc.fillColor(C.dark).fontSize(8).font('Helvetica-Bold')
           .text(value, L + c1W * 0.48, ry, { width: c1W * 0.48 });
      });

      // ── Col 2: Client ──
      doc.rect(c2X, y, c2W, 22).fill(C.navy);
      doc.fillColor(C.white).fontSize(7.5).font('Helvetica-Bold')
         .text('CLIENT', c2X + 10, y + 7);
      ...
      clientRows.forEach(([label, value], i) => {
        ...
        doc.fillColor(C.slate).fontSize(8).font('Helvetica')
           .text(label, c2X + 10, ry, { width: c2W * 0.42 });
        doc.fillColor(C.dark).fontSize(8).font('Helvetica-Bold')
           .text((value).slice(0, 26), c2X + c2W * 0.45, ry, { width: c2W * 0.5 });
      });

      // ══════════════════════════════════════════════════════════════════
      // 4. TECHNICIAN ROW (full width)
      // ══════════════════════════════════════════════════════════════════
      y += 22 + bodyH + 12;

      doc.rect(L, y, CW, 22).fill(C.navy);
      doc.fillColor(C.white).fontSize(7.5).font('Helvetica-Bold')
         .text('TECHNICIEN ASSIGNÉ', L + 10, y + 7);
      ...
      techCols.forEach(({ label, value }, i) => {
        ...
        doc.fillColor(C.dark).fontSize(9.5).font('Helvetica-Bold')
           .text(value.slice(0, 32), tx, y + 42, { width: tcW - 28 });
      });
      ...
      if (dto.resolution) {
        ...
        doc.fillColor(C.white).fontSize(7.5).font('Helvetica-Bold')
           .text('RÉSOLUTION', L + 10, y + 7);

        const resText = dto.resolution;
        const resH = Math.max(
          52,
          doc.heightOfString(resText, { width: CW - 24, lineGap: 3 }) + 22,
        );
        doc.rect(L, y + 22, CW, resH).fill(C.greenLight).stroke(C.greenBorder);
        doc.fillColor('#14532d').fontSize(9.5).font('Helvetica')
           .text(resText, L + 14, y + 32, { width: CW - 28, lineGap: 3 });

        y += 22 + resH;
      }

      // ══════════════════════════════════════════════════════════════════
      // 7. STATS ROW (3 KPI cards)
      // ══════════════════════════════════════════════════════════════════
      y += 14;
      ...
      kpis.forEach(({ value, label, bg, fg, br }, i) => {
        ...
        doc.fillColor(C.slate).fontSize(8).font('Helvetica')
           .text(label, kx, y + 48, { width: kw, align: 'center' });
      });
      ...
      // "SIGNATURES" label with flanking rules
      const lblW = 90;
      ...
      doc.fillColor(C.slate).fontSize(8).font('Helvetica-Bold')
         .text('SIGNATURES', L + ruleW, y, { width: lblW, align: 'center' });
      ...
      doc.fillColor(C.slate).fontSize(8).font('Helvetica')
         .text('Signature du technicien', L, y + 9, { width: sigW, align: 'center' });
      doc.moveTo(L + 18, y + 60).lineTo(L + sigW - 18, y + 60)
         .dash(3, { space: 4 }).strokeColor(C.border).stroke();
      doc.undash();
      doc.fillColor(C.dark).fontSize(8.5).font('Helvetica-Bold')
         .text(tech?.name || '', L, y + 67, { width: sigW, align: 'center' });

      // Client box
      const s2X = L + sigW + 16;
      doc.roundedRect(s2X, y, sigW, 82, 4).fillAndStroke(C.slatePale, C.border);
      doc.fillColor(C.slate).fontSize(8).font('Helvetica')
         .text('Signature du client / représentant', s2X, y + 9, { width: sigW, align: 'center' });
      doc.moveTo(s2X + 18, y + 60).lineTo(s2X + sigW - 18, y + 60)
         .dash(3, { space: 4 }).strokeColor(C.border).stroke();
      doc.undash();
      doc.fillColor(C.dark).fontSize(8.5).font('Helvetica-Bold')
         .text(client.user?.name || '', s2X, y + 67, { width: sigW, align: 'center' });

      // ══════════════════════════════════════════════════════════════════
      // 9. FOOTER (fixed at bottom of page)
      // ══════════════════════════════════════════════════════════════════
      doc.rect(0, 808, W, 34).fill(C.navyDark);
      doc.rect(0, 808, W, 2).fill(C.blue);             // top accent line

      doc.fillColor(C.accent).fontSize(7.5).font('Helvetica')
         .text(
           'DataServ  —  Service Technique & Support Informatique  |  25 Av. Kh., Tunis 1073',
           L, 817, { width: CW * 0.65 },
         );
      doc.fillColor(C.white).fontSize(7.5).font('Helvetica-Bold')
         .text(
           `Généré le ${closeDate}  |  ${ref}`,
           L + CW * 0.65, 817, { width: CW * 0.35, align: 'right' },
         );
